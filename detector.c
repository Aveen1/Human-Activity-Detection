...
#endif
            char buff[256];
            sprintf(buff, "%s/%s_%d.weights","/content/drive/My Drive/backup", base, i);
            save_weights(net, buff);
        }

        if (i >= (iter_save_last + 100) || i % 50 == 0) {
            iter_save_last = i;
#ifdef GPU
            if (ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
            sprintf(buff, "%s/%s_last.weights","/content/drive/My Drive/backup", base);
            save_weights(net, buff);
        }
        free_data(train);
    }
